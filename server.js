...
io.on('connection', (socket) => {
    ...
    socket.on('start-automation', async (data) => {
        try {
            ...
            socket.emit('log', 'Acessando Flow... Aguarde.');
            ...
            try {
                ...
                await sendScreenshot(socket, page, "Erro de Navegação");
                if (browser) await browser.close();
                return;
            }
            ...
            await sendScreenshot(socket, page, "Erro Crítico");
            if (browser) await browser.close();
        }
    });
    ...
    socket.on('confirm-start', async () => {
        ...
        page.on('console', msg => {
            ...
            if (text.startsWith('[FLOW_LOG]')) {
                ...
                socket.emit('log', 'CONSOLE ERROR: ' + text);
            }
        });
            ...
            await page.evaluate(async (promptsList, assetsData) => {
                try {
                    ...
                    async function processPrompts() {
                        log("Iniciando varredura...", "success");
